...
void user_request_handler(
  ...
  std::string response_address = request.has_response_address() ? request.response_address() : "";
    ...
    if (succeed) {
      if (threads.find(wt) == threads.end()) {
        ...
        if (request_type == "GET") {
          ...
          logger->error("Unknown request type {} in user request handler.", request_type);
        }
