...
void process_rep_factor_change(string ip,
  ...
  if (thread_id == 0) {
    ...
    for (unsigned tid = 1; tid < thread_num; tid++) {
      zmq_util::send_string(change_string, &pushers[ServerThread(ip, tid).get_replication_factor_change_connect_addr()]);
    }
  }
    ...
    if (key_stat_map.find(key) != key_stat_map.end()) {
      ...
      if (succeed) {
        ...
        for (int j = 0; j < req.tuple(i).global_size(); j++) {
          ...
          if (curr_global.global_replication() < placement[key].global_replication_map_[curr_global.tier_id()]) {
            decrement = true;
          }

          placement[key].global_replication_map_[curr_global.tier_id()] = curr_global.global_replication();
        }

        for (int j = 0; j < curr_tuple.local_size(); j++) {
          auto curr_local = curr_tuple.local(j);
          if (curr_local.local_replication() < placement[key].local_replication_map_[curr_local.tier_id()]) {
            decrement = true;
          }

          placement[key].local_replication_map_[curr_local.tier_id()] = curr_local.local_replication();
        }
        ...
        if (succeed) {
          if (threads.find(wt) == threads.end()) { // this thread is no longer responsible for this key
          ...
          // decrement represents whether the total global or local rep factor has been
          // reduced; if that's not the case, and I am the "first" thread
          // responsible for this key, then I gossip it to the new threads that
          ...
          logger->error("Missing key replication factor in rep factor change routine.");
        }
      } else {
        logger->error("Missing key replication factor in rep factor change routine.");
        ...
        for (int j = 0; j < curr_tuple.global_size(); j++) {
          placement[key].global_replication_map_[curr_tuple.global(j).tier_id()] = curr_tuple.global(j).global_replication();
        }

        for (int j = 0; j < curr_tuple.local_size(); j++) {
          placement[key].local_replication_map_[curr_tuple.local(j).tier_id()] = curr_tuple.local(j).local_replication();
        }
      }
    } else {
      // just update the replication factor
