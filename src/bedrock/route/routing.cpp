...
void run(unsigned thread_id) {
  ...
  replication_factor_change_puller.bind(rt.get_replication_factor_change_bind_addr());
  ...
  while (true) {
    ...
    if (pollitems[2].revents & ZMQ_POLLIN) {
      replication_response_handler(logger,
                                   &replication_factor_puller,
                                   pushers,
                                   rt,
                                   tier_data_map,
                                   global_hash_ring_map,
    ...
    if (pollitems[3].revents & ZMQ_POLLIN) {
      replication_change_handler(logger,
