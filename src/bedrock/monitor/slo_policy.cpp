...
slo_policy(shared_ptr<spdlog::logger> logger,
  ...
  if (ss.avg_latency > SLO_WORST && adding_memory_node == 0) {
    ...
    if (ss.min_memory_occupancy > 0.15) {
      ...
      if (time_elapsed > GRACE_PERIOD) {
        add_node(logger, "memory", node_to_add, adding_memory_node, management_address);
      }
  ...
  } else if (ss.min_memory_occupancy < 0.05 && !removing_memory_node && memory_node_number > max(ss.required_memory_node, (unsigned)MINIMUM_MEMORY_NODE)) {
